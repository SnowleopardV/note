...
def create_simple_charts(daily_stats):
    """创建简单的折线图，避免中文乱码"""
    ...
    try:
        ...
        ax1.set_title('New vs Cumulative Cases Comparison', fontsize=14, fontweight='bold')
        ...
        axes[1, 1].set_title('Daily New Recovered vs Deaths', fontsize=14, fontweight='bold')
        ...
        chart_file = os.path.join(script_dir, f"HK_COVID_Daily_Stats_{timestamp}.png")
...
def create_chinese_charts_with_font_fix(daily_stats):
    """使用字体修复创建中文图表"""
    if daily_stats is None:
        return
    ...
    try:
        ...
        # 方法2: 使用matplotlib内置的中文字体
        plt.rcParams['font.sans-serif'] = ['DejaVu Sans', 'Arial Unicode MS', 'SimHei']
        plt.rcParams['axes.unicode_minus'] = False
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('香港疫情数据每日统计', fontsize=16, fontweight='bold')
        ...
        plt.savefig(chart_file, dpi=300, bbox_inches='tight')
        print(f"✅ 中文图表已保存到: {chart_file}")
        
        plt.show()
        return chart_file
        
    except Exception as e:
        print(f"❌ 生成中文图表时发生错误: {e}")
        return None
...
def main():
    ...
    if daily_stats is not None:
        # 创建英文图表（避免乱码）
        chart_file1 = create_simple_charts(daily_stats)
        
        # 尝试创建中文图表
        chart_file2 = create_chinese_charts_with_font_fix(daily_stats)
        
        print("\n" + "=" * 60)
        print("✅ 数据分析完成！")
        print("=" * 60)
        if chart_file1:
            print(f"📊 英文图表: {chart_file1}")
        if chart_file2:
            print(f"📊 中文图表: {chart_file2}")
    else:
        print("\n❌ 数据分析失败")
